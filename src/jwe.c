...
void LearnCharComponentsFromFile()
{
    ...
    while((read = getline(&line, &len, fin)) != -1)
    {
        ...
        while(pch != NULL)
        {
            char2comp[id].comp[tmp_cnt++] = pch - MIN_CHINESE;
            pch = strtok_r(NULL, " \n", &save_ptr);
        }
    }
    if(line)
        ...
        if(cw)
        {
            ...
            for(a = b; a < window * 2 + 1 - b; a++)
                if(a != window)
                {
                    ...
                    {
                        if(average_sum == 1)
                            syn0[c + last_word * layer1_size] += neu1_grad[c] / cw + char_list_cnt;
                        else
            ...
            for(a = 0; a < char_list_cnt; a++)
            {
                ...
                {
                    if(average_sum == 1)
                        synchar[c + char_id * layer1_size] += neu1_grad[c] / char_list_cnt + cw;
                    else
